...
df_dof_ms=pd.read_pickle('../data/processed/dof_ms/df_mean_lat_speed_error.pkl')
...
fig, ax = plt.subplots()
ax=sns.lineplot(x=df_dof_ms['lat'], y=df_dof_ms['speed_error'], label='ms',linewidth=1)
ax=sns.lineplot(x=df_jpl['lat'], y=df_jpl['speed_error'], label='jpl',linewidth=1)
