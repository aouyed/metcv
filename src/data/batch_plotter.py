...
def coord_loop(ds_total):
    ...
    for coord in tqdm(coords):
        ...
        for filter_u in ds_total.filter.values:
            print(filter_u)
...
def plot_preprocessor(ds, ds_track, ds_qv_grad, ds_name):
    ...
    ds_total = df_merger_loop(ds, ds_track, ds_qv_grad, dates, ds_name)
    filename = PATH + ds_name+'.nc'
    #ds_total = xr.open_dataset(filename)
    df_results = coord_loop(ds_total)
...
def run(triplet, pressure=500, dt=3600):
    ...
    ds_name = str(dt)+'_' + str(pressure) + '_'+triplet.strftime("%B").lower()
