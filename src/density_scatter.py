...
df_sample=df.sample(n=int(df.shape[0]/1e2), random_state=1)
df_sample_jpl=df.sample(n=int(df.shape[0]/1e2), random_state=1)

ax = dfc.density_scatter(df_sample['speed'], df_sample['speed_approx'], bins=[100, 100], s=1, cmap=sns.cm.rocket)
ax1 = dfc.density_scatter(df_sample_jpl['speed'], df_sample_jpl['speed_approx'], bins=[1000, 1000], s=1, cmap=sns.cm.rocket)